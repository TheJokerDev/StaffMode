package me.thejokerdev.staffmode.utils;
...
public class Utils {
    ...
    public void sendStaffMSG(Player p, String msg){
        String format = formatMSG(p, plugin.getConfigUtil().getConfig().getString("staff-chat.format"));
        format = format.replace("{message}", msg);
        for(Player t : plugin.getServer().getOnlinePlayers()){
            if (!t.hasPermission("staffmode.staff")){
                continue;
            }
            sendMSG(t, format);
            t.playSound(t, Sound.ENTITY_CHICKEN_EGG, 1.0f, 2.0f);
        }
    }
